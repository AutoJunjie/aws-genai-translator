	...
	async function save() {
		...
		try {
			...
			try {
				...
				console.log(cfnOutputs.ReactAppApiUrl)
				console.log("*****here****"+APIENDPOINT)
				const presignedUrlResponse = await fetch(APIENDPOINT+'presignurl', {
					method: 'POST',
					headers: {
				...
				const uploadResponse = await fetch(presignedUrl, {
				  method: 'PUT',
				  body: formState.document.fileInfo, // File
				});
			...
			const jobIdentity = "us-east-1:de80c603-f39b-c92b-7a72-45881de6ed93"
			...
			const jobInfo = {
				...
				jobStatus: "UPLOADED",
				sourceKey: jobId + "/upload/" + formState.document.name
			};
			// Output to console
